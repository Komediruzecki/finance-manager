...
- Bills and Housing now share `ToggleField` for Autopay: an unframed two-column settings row with a stable title/subtitle stack, right-aligned branded switch, and `aria-labelledby`/`aria-describedby` wiring. This removes the redundant refresh icon and prevents the previous inline spans from colliding at narrow modal widths.
