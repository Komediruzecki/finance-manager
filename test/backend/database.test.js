...
describe('Database Schema', () => {
  test('profiles table exists with correct columns', () => {
...
describe('Database Operations', () => {
  ...
  test('can create categories with profile isolation', () => {
    ...
    ).run('Housing', '#ef4444', 'expense', 1);
    db.prepare(
      'INSERT INTO categories (name, color, type, profile_id) VALUES (?, ?, ?, ?)'
    ).run('Salary', '#10b981', 'income', 1);
  ...
  test('profile isolation works', () => {
    ...
    db.prepare(
      'INSERT INTO categories (name, color, type, profile_id) VALUES (?, ?, ?, ?)'
    ).run('Rent', '#000000', 'expense', profile2.id);
