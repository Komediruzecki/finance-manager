...
module.exports = function({ db, apiRateLimiter, logError, spreadsheetService, pdfService, pdfRenderService, requireAuth }) {
  ...
  function sanitizeInput(input) {
    ...
    const commandInjectionPatterns = [
      ...
      />\s*(\||>>|>|<|&)/, /~\//, /etc\/(?:passwd|shadow|group)/,
      /home\/(?:root|admin)/, /\/(dev|proc|sys)\//, /sudo/, /ping\s+-/,
    ];
    ...
    const dangerousSQLPatterns = [
      /DROP\s+TABLE/i, /DROP\s+DB/i, /DROP\s+DATABASE/i, /DELETE\s+FROM/i,
      /INSERT\s+INTO/i, /UPDATE\s+\w+/i, /TRUNCATE/i, /ALTER\s+\w+/i,
      /\.\./i, /\*/i,
    ];
